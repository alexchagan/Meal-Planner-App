...
class TestMealsRoutes(TestCase):
    ...
    def test_send_weekly_meals(self):
        # Prepare test data
        today = date.today().strftime('%Y-%m-%d')
        meal1 = MealSQL(user_id=1, date=today, type='common', period='morning', meal='Oatmeal', serving=100, cal=300, protein=10, carb=50, fat=5)
        meal2 = MealSQL(user_id=1, date=today, type='common', period='afternoon', meal='Chicken Breast', serving=150, cal=400, protein=30, carb=10, fat=10)
        meal3 = MealSQL(user_id=1, date=today, type='common', period='evening', meal='Salmon', serving=200, cal=500, protein=40, carb=5, fat=20)
    ...
    def test_remove_meal(self):
        # Prepare test data
        today = date.today().strftime('%Y-%m-%d')
        meal = MealSQL(user_id=1, date=today, type='common', period='morning', meal='Oatmeal', serving=100, cal=300, protein=10, carb=50, fat=5)
        db.session.add(meal)
        db.session.commit()
        ...
        meal = MealSQL.query.filter_by(user_id=1, date=today, period='morning', meal='Oatmeal').first()
