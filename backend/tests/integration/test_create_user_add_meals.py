...
class TestMealRoutes(TestCase):
    ...
    def test_create_meal_for_user(self):
        ...
        morning_meal = MealSQL.query.filter_by(user_id='user123', date=current_date, period='morning', meal='Oatmeal').first()
        self.assertIsNotNone(morning_meal)
        self.assertEqual(morning_meal.serving, 100)

        afternoon_meal = MealSQL.query.filter_by(user_id='user123', date=current_date, period='afternoon', meal='Chicken Breast').first()
        self.assertIsNotNone(afternoon_meal)
        self.assertEqual(afternoon_meal.serving, 150)

        evening_meal = MealSQL.query.filter_by(user_id='user123', date=current_date, period='evening', meal='Salmon').first()
